...
public class FacultyCtl extends BaseCtl {
	...
	/*	if (DataValidator.isSpace(request.getParameter("firstName"))) {
		...
		if (DataValidator.isNull(request.getParameter("qualification"))) {
			request.setAttribute("qualificatio", PropertyReader.getValue("error.require", "Qualification"));
			pass = false;
		}else if (!DataValidator.isName(request.getParameter("qualification"))) {
			request.setAttribute("qualificatio", PropertyReader.getValue("Qualification Name must be String"));
			pass = false;
		}
...
System.out.println("bean" + bean);
			try {
				if (id > 0) {
					...
					System.out.println("succcesfuly");
				} else {
