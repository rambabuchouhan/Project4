...
<body>
 ...
 --%>				<tr>
					<td align="center"><label>First Name :</label> <input
						...
						rollno="<%=ServletUtility.getParameter("login", request)%>">
						&emsp; Role <%=HTMLUtility.getList("roleId", ServletUtility.getParameter("roleId", request), l)%>

						<input type="submit" name="operation"
						value="<%=UserListCtl.OP_SEARCH%>"> <input type="submit"
						name="operation" value="<%=UserListCtl.OP_RESET%>">&emsp;<%-- <label>Date Of Birth :</label><input type="text" name="MobileNumber" size="23"
						placeholder="Enter Date Of Birth"
						value="<%=ServletUtility.getParameter("dob", request)%>"> --%></td>
     ...
     <%-- <%} %> --%>
			...
			<table width="100%">
				<tr>
					...
					<%
					System.out.println(model.nextPK()-1+"  ==  "+bean.getId());
